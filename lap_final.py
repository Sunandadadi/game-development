...
class Grid(object):
    ...
    @staticmethod
    def generate_puzzle(nrows, ncols, nregions, limit=1000):
        # initialize starting array
        startingarray = np.full((nrows, ncols), "a")
        ...
        # start random walks
        maxperregion = (nrows * ncols) / nregions
        tries = 0
        while tries < limit:
            for r in regions[1:]:
                while True:
                    start_coord = randint(0, nrows - 1), randint(0, ncols - 1)
                    if startingarray[start_coord] == "a":
                        break
                startingarray[start_coord] = r

                while True:
                    r_count = (startingarray == r).sum()
                    if r_count == maxperregion:
                        break
                    try:
                        ...
                        if startingarray[next_coord] in ["a", r]:
                            tries += 1
                            if tries > limit:
                                break
                            orig_value = startingarray[next_coord]
                            startingarray[next_coord] = r
                            if not GridChecks.check_a_contiguity(startingarray):
                                startingarray[next_coord] = orig_value
                            else:
    ...
    def sixbysix(hardcoded=False):
        if hardcoded:
            ...
            while True:
                p = Grid.generate_puzzle(6, 6, 4)
                if GridChecks.perform_checks(3, p):
                    if GridChecks.check_contiguity(p):
                        final_puzzle = []
                        for r in p:
                            row = "".join(r)
                            final_puzzle.append(row)
                        return final_puzzle
    ...
    def eightbyeight(hardcoded=False):
        if hardcoded:
            ...
            while True:
                p = Grid.generate_puzzle(8, 8, 4)
                if GridChecks.perform_checks(4, p):
                    if GridChecks.check_contiguity(p):
                        final_puzzle = []
                        for r in p:
                            row = "".join(r)
    ...
    def twelvebyfifteen(hardcoded=False):
        if hardcoded:
            ...
            try_count = 0
            while True:
                p = Grid.generate_puzzle(12, 15, 9, limit=1000000)
                #try_count += 1
                #print(try_count)
                #for r in p:
                    #print(r)
                if GridChecks.perform_checks(8, p):
                    if GridChecks.check_contiguity(p):
                        final_puzzle = []
                        for r in p:
                            row = "".join(r)
...
class Lap(object):
    ...
    def construct_from_history(self, history):
        ...
        history.reverse()
        for h in history:
            if h.get('player_num') == self.player_order and h.get('result') == 'You have guessed incorrectly.':
                self.remove_result_from_possibilities(h['move'])

            elif h.get('result').startswith('clue') and h.get('player_num') == self.player_order:
                move = h.get('move')
    ...
    def fetch_board(self):
        if self.game_id == Constants.sixbysix:
            return Grid.sixbysix(hardcoded=True)
        elif self.game_id == Constants.eightbyeight:
            return Grid.eightbyeight(hardcoded=True)
        elif self.game_id == Constants.twelvebyfifteen:
            return Grid.twelvebyfifteen(hardcoded=True)
...
if __name__ == '__main__':
    game_id = Constants.eightbyeight
    lp = Lap(game_id)
    lp.start_game()
